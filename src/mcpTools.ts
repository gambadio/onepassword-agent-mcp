...
export function createMcpServer(): McpServer {
  ...
  server.registerTool(
    "find_passwords_for_site",
    {
      ...
      description:
        "Return approved encrypted password handles for a website. The handle is opaque and does not contain plaintext for the model.",
      inputSchema: {
  ...
  server.registerTool(
    "list_approved_passwords",
    {
      ...
      description:
        "List locally approved password handles and allowed sites. Does not return plaintext passwords.",
      inputSchema: {},
    },
    async () => {
      const grants = await policy.listPublicGrants();
  ...
  server.registerTool(
    "copy_password",
    {
      ...
      description:
        "Resolve an encrypted password handle at the last moment, copy the password to the OS clipboard, and return no plaintext.",
      inputSchema: {
    ...
    async ({ encryptedPasswordHandle, expectedSite, clearAfterSeconds }) => {
      ...
      return textResponse(
        `Password for "${grant.title}" copied to clipboard and scheduled to clear. Plaintext was not returned.`,
      );
    },
  );
  ...
  server.registerTool(
    "paste_password",
    {
      ...
      description:
        "Resolve an encrypted password handle, copy it to the clipboard, paste into the active app, and return no plaintext.",
      inputSchema: {
        encryptedPasswordHandle: z.string().describe("Opaque handle returned by find_passwords_for_site."),
        expectedSite: z.string().optional().describe("Current website/origin. Required unless local policy allows omission."),
        clearAfterSeconds: z.number().int().min(1).max(300).optional(),
