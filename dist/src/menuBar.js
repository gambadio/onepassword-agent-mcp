...
export async function installMenuBar(settings, options = {}) {
    assertMacOS();
    if (!hasSwiftCompiler()) {
    ...
    if (!existsSync(iconSource))
        throw new Error(`Menu-bar icon is missing: ${iconSource}`);
...
export async function uninstallMenuBar(settings, options = {}) {
    assertMacOS();
    const targetApp = menuBarAppPath();
    assertSafeAppPath(targetApp);
...
export async function setMenuBarLaunchAtLogin(enabled) {
    assertMacOS();
    ...
    if (!existsSync(appPath))
        throw new Error("Install the menu-bar companion before enabling launch at login.");
...
export function launchMenuBar() {
    assertMacOS();
    const appPath = menuBarAppPath();
    if (!existsSync(appPath))
        throw new Error("The menu-bar companion is not installed.");
    const result = spawnSync("/usr/bin/open", ["-g", appPath], { encoding: "utf8" });
    if (result.error)
        throw result.error;
    if (result.status !== 0)
        throw new Error(`Could not open the menu-bar companion: ${result.stderr.trim()}`);
}
export function quitMenuBar() {
...
export function launchAgentPlist(appPath) {
    return `<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
...
function assertMacOS() {
    if (process.platform !== "darwin")
        throw new Error("The optional menu-bar companion is available on macOS only.");
}
function assertSafeAppPath(value) {
