...
export function clientLabel(target) {
    switch (target) {
...
export function resolveMcpLaunch() {
    ...
    if (override)
        return { command: override, args: ["mcp"] };
...
export function isClientDetected(target) {
    switch (target) {
        case "claude-code":
        ...
        case "xcode":
            return process.platform === "darwin" && (appExists("Xcode.app") || existsSync(xcodeRoot()));
        case "raycast":
...
export function setupClient(target, scope) {
    const launch = resolveMcpLaunch();
    const label = clientLabel(target);
    if (!isClientDetected(target))
        return { label, status: "not-detected" };
    try {
        switch (target) {
            case "claude-code": {
                ...
                if (get.ok)
                    run(command, ["mcp", "remove", "--scope", scope, serverName]);
...
export function uninstallClient(target, scope) {
    const label = clientLabel(target);
    try {
        switch (target) {
            case "claude-code": {
...
export function setupPlan(target, scope) {
    const launch = resolveMcpLaunch();
    switch (target) {
        case "claude-code":
...
export function genericMcpServersConfig(launch = resolveMcpLaunch()) {
    return {
        mcpServers: {
            [serverName]: {
...
export function vscodeWorkspaceConfig(launch = resolveMcpLaunch()) {
    return {
        servers: {
            [serverName]: {
                type: "stdio",
...
export function setJsonMcpServer(configPath, rootKey, launch, includeType) {
...
export function removeJsonMcpServer(configPath, rootKey) {
    if (!existsSync(configPath))
        return false;
    const config = readJsonObject(configPath);
    const currentRoot = readServerRoot(config, rootKey, configPath);
    if (!(serverName in currentRoot))
        return false;
...
export function vscodeConfigPath(platform = process.platform, home = os.homedir(), env = process.env) {
    if (platform === "win32") {
...
function setupXcode(launch) {
    ...
    if (codex) {
        ...
        if (!result.ok)
            throw new Error(`Xcode Codex setup failed: ${result.output}`);
        configured = true;
...
function openRaycast(command, detail) {
...
function readJsonObject(configPath) {
    if (!existsSync(configPath))
        return {};
...
function readServerRoot(config, rootKey, configPath) {
...
function readServerEntry(value, rootKey, configPath) {
    if (value === undefined)
        return {};
    if (value && typeof value === "object" && !Array.isArray(value))
        return value;
...
function resolveXcodeAgentCli(agent) {
...
function xcodeCodexHome() {
    return path.join(xcodeRoot(), "codex");
}
function xcodeClaudeHome() {
...
function formatLaunch(launch) {
    return [launch.command, ...launch.args].join(" ");
}
function errorMessage(error) {
    return error instanceof Error ? error.message : String(error);
}
//# sourceMappingURL=clientSetup.js.map