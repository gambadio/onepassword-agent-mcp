...
export function createMcpServer(): McpServer {
  ...
  const findSecrets = async (site: string) => {
  ...
  const copySecret = async (handle: string, expectedSite?: string, clearAfterSeconds?: number) => {
    const file = await store.load();
    ...
    return textResponse(
      `Secret field "${grant.fieldLabel}" for "${grant.itemTitle || grant.title}" copied to clipboard and scheduled to clear. Plaintext was not returned.`,
    );
  };
  ...
  const pasteSecret = async (handle: string, expectedSite?: string, clearAfterSeconds?: number) => {
    const file = await store.load();
    const { grant, secretRef } = await policy.resolveHandle(handle, expectedSite);
    const secret = await new OpCli(file.settings).readSecret(secretRef);
    await setClipboard(secret, clearAfterSeconds || file.settings.clipboardClearSeconds);
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      if (input.approveForAgents) {
        ...
        if (!selected) throw new Error("The saved item does not have an approvable field.");
...
function publicSecretGrant(grant: Awaited<ReturnType<PolicyService["listPublicGrants"]>>[number]) {
  return {
    id: grant.id,
    title: grant.title,
...
function selectCandidateToApprove(
  candidates: ReturnType<OpCli["itemToCandidates"]>,
  selector?: string,
) {
  if (!selector) {
